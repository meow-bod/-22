...
const AdminDashboardClient = ({ initialApplicants, usersWithPets }: AdminDashboardClientProps) => {
  ...
  const handleApproval = async (applicantId: string, newStatus: boolean) => {
    ...
    startTransition(async () => {
      const { error } = await supabase.from('sitters').update({ is_approved: newStatus }).eq('id', applicantId);
      ...
      if (error) {
        ...
        setApplicants(applicants.map(app => (app.id === applicantId ? { ...app, is_approved: newStatus } : app)));
        router.refresh();
      }
    });
  };
              ...
              {applicants.map(applicant => (
                <tr key={applicant.id}>
                  ...
                  <td className='px-6 py-4 whitespace-nowrap'>
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${applicant.is_approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
                    >
